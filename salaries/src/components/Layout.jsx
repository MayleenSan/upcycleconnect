...
import { LayoutDashboard, BookOpen, Calendar, Lightbulb, MessageSquare, User, LogOut, Menu, X } from 'lucide-react'

const nav = [
  { label: 'Tableau de bord', chemin: '/', icone: LayoutDashboard },
  { label: 'Formations', chemin: '/formations', icone: BookOpen },
  { label: 'Mon planning', chemin: '/planning', icone: Calendar },
  { label: 'Conseils & News', chemin: '/conseils', icone: Lightbulb },
  { label: 'Forums', chemin: '/forums', icone: MessageSquare },
    ...
    <div className="min-h-screen bg-[#F8F4EE] flex">
      <aside className="hidden md:flex flex-col w-60 bg-white border-r border-gray-100 fixed h-full">
        <div className="p-5 border-b border-gray-100 flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-xs text-gray-400">Espace Salariés</span>
          </div>
        </div>
          ...
          {nav.map((item) => {
            ...
            return (
              <Link key={item.chemin} to={item.chemin}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${actif ? 'bg-[#2D6A4F] text-white font-medium' : 'text-gray-600 hover:bg-[#F8F4EE]'}`}>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 bg-white border-b border-gray-100 z-20 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-[#2D6A4F] text-sm">Salariés</span>
        </div>
